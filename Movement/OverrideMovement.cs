...
/// <summary>
/// Hooks RMIWalk and RMIFly to inject movement input for automated navigation.
/// </summary>
public unsafe class OverrideMovement : IDisposable
{
    ...
    /// <summary>
    /// Distance threshold for considering position reached.
    /// </summary>
    public float Precision = 0.01f;

    /// <summary>
    /// True if player (or some other plugin) is pressing movement keys.
    /// </summary>
    ...
    {
        var rmiWalkIsInputEnabled1Addr = Services.SigScanner.ScanText("E8 ?? ?? ?? ?? 84 C0 75 10 38 43 3C");
        var rmiWalkIsInputEnabled2Addr = Services.SigScanner.ScanText("E8 ?? ?? ?? ?? 84 C0 75 03 88 47 3F");
        Services.Log.Information($"RMIWalkIsInputEnabled1 address: 0x{rmiWalkIsInputEnabled1Addr:X}");
        Services.Log.Information($"RMIWalkIsInputEnabled2 address: 0x{rmiWalkIsInputEnabled2Addr:X}");
        ...
        Services.Log.Information($"RMIWalk address: 0x{_rmiWalkHook.Address:X}");
        Services.Log.Information($"RMIFly address: 0x{_rmiFlyHook.Address:X}");
        Services.GameConfig.UiControlChanged += OnConfigChanged;
        UpdateLegacyMode();
    }
    ...
    {
        _legacyMode = Services.GameConfig.UiControl.TryGetUInt("MoveMode", out var mode) && mode == 1;
        Services.Log.Info($"Legacy mode is now {(_legacyMode ? "enabled" : "disabled")}");
    }
}
